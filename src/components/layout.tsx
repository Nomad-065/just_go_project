...
const Layout = () => {
  ...
  return (
    <div className="h-screen w-full flex flex-col bg-justgo-green">
      ...
      <div className={'flex-1 flex w-full h-full overflow-hidden pl-1 pr-2'}>
        {/* Sidebar */}
        <div
          className={cn(
            'flex flex-col py-4 items-end justify-start gap-2 transition-all duration-300 overflow-hidden flex-shrink-0 ',
            isSidebarExpanded
              ? 'w-50 '
              : 'w-12'
          )}
        >
          {navMenu.map(({to, label, icon: Icon, children}) => {
            ...
            return (
              <Tooltip key={to} title={label} placement="right" arrow>
                <NavLink className={'flex w-full'} to={to}>
                  {({isActive}) => (
                    <div
                      className={cn(
